...
config/claude-permission-mode  optional one-token permission posture for every Claude worker launch: absent or "auto" launches with --permission-mode auto, "bypass" is the deliberate opt-in to --dangerously-skip-permissions; LOCAL, gitignored; inherited by secondmate homes; the session-start digest names the posture in force and what selected it; see docs/configuration.md "Claude permission mode"
...
config/board-live-port  optional pinned port for this home's live bearings board server; LOCAL, gitignored; absent means a port derived from the home's own path, stable across restarts and distinct per home; see docs/configuration.md "Live bearings board"
...
state/               runtime records and signals; gitignored
  ...
  board-live.jsonl board-live.endpoint board-live.pid board-live.log  the bearings board's append-only fleet-event log, the running server's endpoint and process, and what it said if it could not start; the log is the durability and the websocket only the latency (docs/configuration.md "Live bearings board")
  board-live.token board-inbound.jsonl  what proves an answer came from a board built in this home, and every authenticated answer as it arrived, without its token; the same connection carries the captain's click back (docs/configuration.md "Live bearings board")
  ...
  fleet-obligations.check.sh  generated fleet obligation poll shim and its .check-trust binding; armed by bootstrap on a locked session once the home has a task or a board; its report record .fleet-obligations is what keeps one owed obligation from being reported on every poll (bin/fm-obligation-check.sh; docs/configuration.md "Fleet obligation report")
  ...
  .hash-* .count-* .stale-* .stale-since-* .churn-since-* .paused-* .wedge-escalations-* .dead-reported-* .pr-head-reported-* .writing-* .waiting-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
  .watch-triage.log  watcher's absorbed-wake debug log (size-capped); never relied on, safe to delete
  .last-watcher-beat watcher liveness beacon, rewritten every poll (including while absorbing benign wakes); guard scripts read its mtime, while its `<pid> <cycle>` content lets an observer see a cycle boundary without waiting for an mtime second to turn over
...
When no-mistakes is selected, no-mistakes alone owns review, fixes, tests, documentation, push, PR, and CI; the approving reviewer below is the one dispatch outside that ownership, because its approval is read off the forge at merge time rather than produced by any pipeline.
The path's worker, automated gates, and captain approval remain authoritative:

- **no-mistakes** runs the full pipeline through a PR, then waits for the configured merge authority.
- **direct-PR** has the worker push and open a PR without the no-mistakes pipeline; a reviewer crewmate then reviews that PR and posts its findings on it, firstmate rules on every finding and posts the ruling there, fixes land as commits on the same PR one finding per commit, and the merge gate is checks green plus every finding ruled, on top of the approval below, after which the configured merge authority applies.
- **local-only** has the worker stop with a clean ready branch, then waits for the configured merge authority before firstmate uses the guarded fast-forward merge path.

Every PR merge, whatever the delivery mode, also requires a reviewer's approval of the exact commit that would merge; `bin/fm-pr-merge.sh` enforces it, no flag waives it, and a worker never approves its own work.
Dispatching that reviewer is mandatory in every mode, and again after each round of fixes, because a fix moves the commit the approval is bound to.
An approval is one more gate before a pull request reaches the captain, never a substitute for his word.

Load `pr-review` before dispatching a reviewer in any delivery mode, on a wake reporting a review posted, and before ruling on, relaying, or fixing a finding; it owns that path's detail and this section does not restate it.
Never stack a manual review of your own on top of the path's own review, hold work for a personal clean verdict, or infer review authority from security, architecture, or risk alone; the approving reviewer above is that required dispatch, not a review of your own.
A separate review or audit beyond the path's own and that required approval is allowed only when the captain explicitly requests that deliverable or the authorized task is a knowledge-only review; one named question remains scoped to that question.
...
When the escalation reaches the captain through the bearings board, its card carries the pull request's own links - the pull request, its checks, its commits, and its review comments - so the captain reads the repository rather than a summary written for him.
...
3. For `check:`, act on the named poll result, including merges, contribution signals, Relay events, process-to-event source results, unmet fleet obligations, and captain inbox notes; a handled inbox note is also acknowledged with `bin/fm-inbox.sh drain --ack <id>`, or it stays counted as still waiting for firstmate.
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `PRESENTATION_UNAVAILABLE:`, `PRESENTATION_UNSTABLE_URL:`, `CLAUDE_PERMISSIONS:`, `BACKEND_INVALID:`, `BACKEND_EXPERIMENTAL:`, `ACTIONS_DORMANT:`, `ACTIONS_UNVERIFIED:`, `NEEDS_GH_AUTH`, `TANGLE:`, `STARTUP_MEMORY_BUDGET:`, `CREW_DISPATCH: invalid`, `FLEET_SYNC:`, `NETWORK_CHECKS:`, `HOME_SUMMARY:`, `BACKLOG_RECONCILE:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `SECONDMATE_HANDOFF:`, `NUDGE_SECONDMATES:`, or `FMX:`), or when `BOOTSTRAP_INFO:` says an interrupted backlog cleanup may have left an endpoint or local copy; silence and other `BOOTSTRAP_INFO:` facts need no load.
...
- `pr-review` - load before dispatching a reviewer against an open pull request in any delivery mode, on a wake reporting that a review was posted, before ruling on, relaying, or fixing a reviewer finding, and before merging a pull request with no approving review at its current head.
