...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  <id>/board-card.json  the durable Captain's Call card for a task held for the captain, written once per hold - seeded when the hold is created, retired and re-seeded when the task is held again - and replaced by what the board last published; bin/fm-captain-hold.sh owns it, and it is what lets the board refresh itself with no model in the loop
