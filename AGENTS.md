...
state/               runtime records and signals; gitignored
  ...
  board-live.jsonl board-live.endpoint board-live.pid board-live.code board-live.log  the bearings board's append-only fleet-event log, the running server's endpoint, process and the code that process is running, and what it said if it could not start; the log is the durability and the websocket only the latency (docs/configuration.md "Live bearings board")
...
Use `gh-axi` for GitHub and `chrome-devtools-axi` for browser work; the captain's one board that the `bearings` skill owns is built, served and answered by this home itself and depends on no external command, while compatible `lavish-axi` is only for a Lavish review session and never for the board; consult current help rather than memorizing flags.
