...
Use `gh-axi` for GitHub and `chrome-devtools-axi` for browser work; the captain's one board that the `bearings` skill owns, and every other page firstmate renders for the captain, are served by this home itself and need no external tool; consult current help rather than memorizing flags.
