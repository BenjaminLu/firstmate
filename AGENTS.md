...
state/               runtime records and signals; gitignored
  ...
  board-remote/      the remote board's awaited card set, its durable answer cursor, and the pre-arm answer snapshot that keeps an earlier round's answer from settling a freshly armed card; written only by bin/fm-procevent-board-remote.sh, and the cursor is never pruned because the board keeps a document until the captain changes it, so dropping it re-delivers answers already acted on (section 13's process-event-sources trigger)
...
A ship task's `done:` or `needs-decision:` event is not ready until `bin/fm-packet.sh verify <id>` passes; when it fails, steer the worker to complete the packet before relaying anything, and relay the packet's substance and path with the outcome.
...
- `process-event-sources` - load before arming a process-event source, before registering a deterministic condition->action watch (do X as soon as Y is true), on any `procevent <adapter> <source-id> <sequence>` check wake, and on any `process-event source stranded` or `process-event source failed to start` check wake.
