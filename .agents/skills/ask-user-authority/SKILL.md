...
4. Count the fix rounds already spent on the step this finding came from.
   ...
   Sending a narrow remedy from that round on anyway stays yours to choose, but the instruction has to say so: name which round the step is on and state that the narrow remedy is deliberate.
   A worker refuses one that does not and appends `blocked:` instead, because the no-mistakes Definition of done in `bin/fm-dod-lib.sh` makes that acknowledgement one of the two answers that end its refusal; the other is the coherent-change instruction it asked for.
