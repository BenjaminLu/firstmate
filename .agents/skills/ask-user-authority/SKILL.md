...
4. Count the fix rounds already spent on the step this finding came from.
   ...
   A worker refuses one that does not and appends `blocked:` instead, because the no-mistakes Definition of done in `bin/fm-dod-lib.sh` makes that acknowledgement the only thing that ends its refusal.
