...
4. Count the fix rounds already spent on the step this finding came from.
   ...
   Nothing carries a question to the reviewer and returns a reply, so never wait for one: `no-mistakes axi respond --action fix --findings` naming every finding that gate is holding, plus `--instructions`, reaches the next fix round, and the only thing that comes back is the next gate report.
