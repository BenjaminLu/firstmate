...
description: >-
  ...
  Covers the knowledge-placement decision tree, the one-owner rule for contracts, the inline-stub pattern for content moved into a skill, AGENTS.md size discipline, trigger hygiene for new skills, the defect classes every change and every review is checked against, and repo style rules (one sentence per line, plain dash, no agent co-author, bin/fm-lint.sh before a script change is done, colocated tests, and maintainer-verification evidence).
