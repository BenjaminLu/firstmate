...
On a clear verdict it prints one operator-facing clause to stderr naming what it checked and what it did not - not only on a failure, because the spawn discards its stdout and a clean verdict is when the unexamined part matters most, but only on a host that actually carries a user-global or ancestor memory file, so an ordinary dispatch on a host with neither gains no output.
