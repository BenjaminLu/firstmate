...
What it scans is the project memory chain of the directory the pane starts in, followed to Claude Code's documented five-file import depth; a hop past that depth is where the product stops loading too, so it is cleared rather than reported undecided, and the clear line names the depth.
Every clear line also states that the operator's own user-global `~/.claude` memory chain was NOT examined, because whether that chain can raise the same dialog was never reproduced here - an unscanned dimension is reported, never left to read as a checked one.
`unknown` covers a chain it could not read to the end, and a launch whose consent entry it could not identify at all - a secondmate home inside a repository whose primary checkout cannot be resolved, where the store lookup would answer about the wrong key.
...
A visible external-imports dialog now means the pre-launch gate missed it, since a launch it can see coming is refused instead; `fm-control.sh <id> interrupt` delivers Escape, which dismisses whichever of the two is on screen without answering it, and is the safe way to clear a wedged pane for inspection.
