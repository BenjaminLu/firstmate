...
- Fill every `{FILL: ...}` slot with the value its hint names: the card prose the rules below require, and each decision card's `reversible`, `risk`, and `recommend_value`, which the skeleton carries as slots rather than leaving to your memory. Add `evidence` to each decision card, plus `packet_url` when the packet is served somewhere linkable.
- A card the skeleton seeded from a verified packet already carries the worker's `decide`, `consequence`, `if_nothing`, `reversible`, `risk`, `recommend_value`, and `recommend_why`, plus the page's `packet_url` when `bin/fm-packet.sh serve <task-id>` ran before compose; keep that substance and translate it rather than rewriting it. Only the fields the packet left out arrive as `{FILL: ...}` slots.
...
Those acknowledgements are yours to settle, and settling them is part of handling the wake, not an optional courtesy: EVERY key the answer named is settled before you finish, whatever kind of key it was - a decision card key, a `merge.<task-id>`, or each id inside a `dispatch.charted` - and there are exactly two outcomes that settle one.
...
A key you leave unsettled keeps reporting itself as still waiting, with the elapsed time, which is what tells the captain a slow answer apart from a missed one - and on work you already acted on that report is a false alarm on the one signal he asked for, so leaving a record behind is a defect, not untidiness.
