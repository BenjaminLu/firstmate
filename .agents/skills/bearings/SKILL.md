...
- Rank, trim, and drop rows the way the chat digest does; the skeleton carries every row the snapshot exposed, and its `charted_more` and `charted_warning_more` slots both name the SAME omitted-gates total - divide that one number between the two counts by kind rather than writing it into each, then add the rows you cut yourself.
