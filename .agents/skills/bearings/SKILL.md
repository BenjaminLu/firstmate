...
It never discovers pull requests - that is the opt-in you pass - so it carries forward the merge cards your last build published instead of dropping the captain's Merge now control; one retires when its PR or its task reaches the payload's landed rows, or when a build whose PR view was COMPLETE finds it no longer merge-ready. A view that lost a repo to a failed or capped `gh` call, never queried some repos, or could not read the backlog is partial, and a partial view carries the merge cards forward rather than retiring them - so if you want a stale merge card gone, give the snapshot a PR view it can complete.
