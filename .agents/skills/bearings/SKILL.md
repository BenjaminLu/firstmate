...
- That card also carries `packet`: the packet itself, drawings included, which the board opens inside the card as a tab per option plus one collapsed block. It is read from the packet, never typed, so leave it exactly as the skeleton carried it - and do not run `bin/fm-packet.sh serve` to give the captain a page, because the card he is already reading holds the whole packet. Its headings already carry all three languages and switch with the rest of the board; only the lines the worker typed reach him in the language they wrote them in. `bin/fm-packet.sh` owns why, and translating those here would put your words in the worker's mouth.
