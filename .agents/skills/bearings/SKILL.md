...
- When the held task has a verified decision packet, start the card from `bin/fm-packet.sh card <task-id>`: it carries the worker's `decide`, per-option `consequence`, `if_nothing`, `reversible`, `risk`, and `recommend_why`; serve the page with `bin/fm-packet.sh serve <task-id>` first so the card also carries the page's `packet_url`, then add `about` and `evidence` yourself and keep the copy trilingual where the packet only has English.
