...
- That card also carries `packet`: the packet itself, drawings included, which the board opens inside the card as a tab per option plus one collapsed block. It is read from the packet, never typed, so leave it exactly as the skeleton carried it - and do not run `bin/fm-packet.sh serve` to give the captain a page, because the card he is already reading holds the whole packet. The prose in that block reaches him in the language the worker wrote it in; `bin/fm-packet.sh` owns why, and translating it here would put your words in the worker's mouth.
