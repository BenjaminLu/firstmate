...
- Rank, trim, and drop rows the way the chat digest does; the skeleton carries every row the snapshot exposed, and its `charted_more` and `charted_warning_more` slots each name how many gate rows the snapshot itself omitted - split that number between the two counts by kind and add the rows you cut.
