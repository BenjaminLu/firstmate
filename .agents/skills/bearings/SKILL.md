...
- A card the skeleton seeded from a verified packet already carries the worker's `decide`, `consequence`, `if_nothing`, `reversible`, `risk`, and `recommend_why`, plus the page's `packet_url` when `bin/fm-packet.sh serve <task-id>` ran before compose; keep that substance and translate it rather than rewriting it.
