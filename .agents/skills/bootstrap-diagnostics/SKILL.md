...
- `PRESENTATION_UNSTABLE_URL: <why the board gets no stable address>` - the installed Lavish build is compatible but does not accept `--name`, so the board comes up at a generated session URL that changes rather than at one stable address.
  ...
  Visual work still functions, so do not hold dispatch; tell the captain that the one-address rule cannot be kept until a build carrying the flag is installed, and let them decide.
