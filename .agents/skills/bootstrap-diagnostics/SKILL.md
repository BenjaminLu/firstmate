...
description: >-
  ...
  Use whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line - MISSING, MISSING_MANUAL, PRESENTATION_UNAVAILABLE, PRESENTATION_UNSTABLE_URL, CLAUDE_PERMISSIONS, BACKEND_INVALID, BACKEND_EXPERIMENTAL, ACTIONS_DORMANT, ACTIONS_UNVERIFIED, NEEDS_GH_AUTH, TANGLE, STARTUP_MEMORY_BUDGET, CREW_DISPATCH invalid, FLEET_SYNC, NETWORK_CHECKS, HOME_SUMMARY, BACKLOG_RECONCILE, SECONDMATE_SYNC, SECONDMATE_LIVENESS, SECONDMATE_HANDOFF, NUDGE_SECONDMATES, or FMX - or reports that an interrupted backlog cleanup may have left an endpoint or local copy, or when a standalone bin/fm-bootstrap.sh or bin/fm-startup-network.sh run prints one of those lines.
...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  For `treehouse`, this also covers an installed version whose `treehouse get` lacks a flag firstmate requires; [`bin/fm-bootstrap.sh`](../../../bin/fm-bootstrap.sh) owns that flag floor, and it is a plain upgrade request.
...
- `MISSING_MANUAL: <tool> (instructions: <url>)` - tell the captain why the tool is required and give them the printed instructions URL, but do not pass the tool to `bin/fm-bootstrap.sh install`; wait for the captain to complete the manual installation, then rerun session start to confirm the dependency is present.
  For `diagram-design`, state the consequence accurately rather than as a blocker: a decision packet's figures are drawn through that skill and `bin/fm-packet.sh verify` refuses a packet that owes figures and has none, but the packet is an optional deliverable rather than an obligation, so nothing is stopped by its absence - it is a dependency to install before a packet is wanted, not work waiting on it.
- `PRESENTATION_UNSTABLE_URL: <why a Lavish session gets no stable address>` - the installed Lavish build is compatible but does not accept `--name`, so a Lavish session comes up at a generated URL that changes rather than at one stable address.
  This too is about the optional Lavish surface and not about the captain's board, which this home serves at a stable address of its own.
  This is not a version problem and must not be answered with an upgrade: the published package does not carry the flag and reports a HIGHER version than the build that does, so upgrading makes it worse.
  Lavish review still functions, so do not hold dispatch; tell the captain that a Lavish session cannot keep one address until a build carrying the flag is installed, and let them decide.
  The `could not read lavish-axi --help` variant means the probe itself did not run, so the address is simply unknown rather than known-bad; re-run once the tool answers `--help`.
- `CLAUDE_PERMISSIONS: <n> ... not pre-approved in <settings> ... merge with: <command>` - this repository ships the command allow-list its own toolchain needs, and the operator's Claude settings do not yet carry all of it, so workers will stop and ask before ordinary fleet commands.
  This follows the same consent rule as a tool install and is never applied for the captain: report what merging does - it only ADDS the listed rules, removes and narrows nothing, and leaves every other setting alone - wait for consent, then run the printed `bin/fm-bootstrap.sh install-permissions`.
  The `could not compare` variant means the comparison did not happen at all; treat it as unknown rather than as a home that needs nothing, and resolve the named settings or interpreter problem first.
- `BACKEND_INVALID: <name> (known: <names>)` - the resolved runtime backend has no verified dependency or lifecycle contract, so do not dispatch work until the invalid `FM_BACKEND` or `config/backend` value is corrected to one of the listed backends.
- `BACKEND_EXPERIMENTAL: <name> (<how it was selected>) - <remedy>` - this home will spawn into an experimental runtime backend, and the parenthesis says which rung of the precedence chose it.
  An `auto-detect` source is the one that matters most: the backend is then a property of whichever terminal firstmate was started in rather than a recorded decision, so two clones of this repository can be running different backends.
  Nothing is broken and dispatch is not held; report the backend and its remedy to the captain, and record their choice in `config/backend` so the next session is deliberate.
- `ACTIONS_DORMANT: <repo> ... <where to enable>` - the repository's checks will never report, so any validation run would reach its CI step and wait out the full CI timeout looking alive while doing nothing.
  This blocks delivery, so do not start a validation run on that repository until it is resolved: the fix is a click in the repository's own Actions settings, which only its owner can make, so ask the captain and give them the printed URL.
- `ACTIONS_UNVERIFIED: <what could not be read>` - whether that repository's checks will report could not be determined, so the delivery risk above is unknown rather than absent.
  Resolve the named read problem and re-run the check before relying on a validation run finishing.
