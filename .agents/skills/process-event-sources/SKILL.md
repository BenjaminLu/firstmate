...
Do the artifact read BEFORE the arm and hand `arm` those same documents. Never ingest and never open a new round over an answer that has not been acted on: `arm` ingests the documents it is given before it opens the cards, so that ordering is the adapter's own and not yours to sequence.
Read its ingest report the way you read a wake's - the answers it delivers there are answers to the round that is ending, and `merge.<task-id>` and `dispatch.charted` among them are yours to route.
An answer settles the card it was given for and no other: once delivered, an answer can never settle a card armed after it, so the same key asked again is a genuinely open question.
...
The adapter itself fetches nothing and depends on no vendor - it is handed a directory of answer documents - so the fetch is the one step a clone running another agent has to supply for itself.
...
`bearings` owns why this source exists at all, the responsiveness a board must not fall below, and the shape of the answer record both boards' readers conform to.
...
: A `board-remote` wake says only that the interval elapsed while a card is still open - it carries no board bytes, so read the board rather than the result. Fetch the answer documents yourself into a directory - on Claude that is the Artifact `read_db`, `db_op list`, collection `answers`, with `out_dir`, and it is the one vendor-bound step in this path - then hand that directory to the adapter:
  ```sh
