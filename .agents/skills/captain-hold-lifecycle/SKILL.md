...
This is not a merge-only habit: whenever the held row still has a worker on it, cleanup owns that row's completion, so `answer` refuses the close and names `--release`, which records the captain's words just the same.
A question-shaped call with no worker of its own still closes normally.
