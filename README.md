...
For Claude Code, nothing is needed to start, and worker panes get their workspace trust registered for them. The step to know about is Claude Code's "Allow external CLAUDE.md file imports?" dialog, which firstmate cannot answer: before each dispatch the gate reads the launch directory's own project memory chain, refuses the dispatch when it can see that chain reaches outside the tree - naming the import and the checkout to approve - and warns but still launches when it cannot tell. It does not read your user-global `~/.claude/CLAUDE.md` or any `CLAUDE.md` above the project, so one of those carrying an outside import can still raise the dialog on a worker. Either way the cure is the same one-time answer: run `claude` in that project's checkout, answer "Yes, allow external imports" once, and every later worker in that project launches straight through.
