...
Those sleeps look like recoverable overhead - the wake-triage cases, then one script and now `fm-watch-triage.test.sh` plus `fm-watch-triage-waits.test.sh`, issue about 1,900 of them between them, each paying a flat ~100ms scheduler wake-up penalty on macOS - but they are not overhead added to the clock; they are how a test waits for a subject that only moves on `fm-watch.sh`'s own one-second `FM_POLL` cadence.
Sampling less often does not remove that wait, it only delays detection: raising the interval to 0.5s and charging each sample proportionally measured those cases, then still one script, at 435s and 440s against 390s and 393s unchanged, back to back on 2026-09-03, because each of its ~40 poll-cycle waits and ~73 process-exit waits paid up to half a second more.
