...
GitLab is gated on its own native approvals instead, which it reports without the commit they cover, so a GitLab merge discloses that its approval is not proven against the head rather than implying the GitHub path's binding.
