...
It accepts GitHub's own `APPROVED` state and, because that state is unreachable from the single account this fleet posts from, equally accepts the verdict a reviewer writes as the last line of a `COMMENTED` review body ([`docs/verification/pr-review.md`](verification/pr-review.md) records both facts, and `bin/fm-brief.sh`'s `--review` contract owns the exact line).
