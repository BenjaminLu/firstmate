...
The captain's `/bearings lavish` board republishes on those same five events, detached and best effort, through `bin/fm-bearings-board.sh refresh`, which recomposes deterministically and injects in place without establishing, rebinding, or re-arming its Lavish session; that script's header owns the mechanics, and `bin/fm-task-progress.sh` owns the per-task progress projection its Underway rows carry.
