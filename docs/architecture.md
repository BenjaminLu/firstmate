...
A canonical validated PR poll reads the pull request's head on the same call that reads its state, so the watcher re-binds the task's recorded `pr_head=` to the live commit on every cycle instead of leaving it on the one captured when the poll was armed; `bin/fm-pr-poll.sh` owns that output contract and `bin/fm-pr-lib.sh`'s `fm_pr_meta_rebind_head` owns the write, which accepts only a commit id the forge just returned and only while the record still names the pull request that head was read for.
An open pull request's moved head is not actionable and wakes nobody.
When that poll returns a merged verdict, the watcher records the pull request's head as the poll last read it and then routes the outcome through the shared merge-outcome emitter before retiring the poll.
`pr_head=` is the head of the pull request's own branch, never the commit created on the base branch: this fleet merges with `--squash` by default (`bin/fm-pr-merge.sh`), and under squash or rebase the commit that lands is a new object that `pr_head=` is not an ancestor of.
That is what its consumers already read it as - `bin/fm-review-diff.sh` diffs against it, `bin/fm-teardown.sh` compares patch ids against it and falls back to a content check precisely because a squashed change lands as a different commit - so a reader must not treat it as a commit on the default branch.
Only an armed merge poll keeps that value on the pull request's current commit, so the fleet snapshot reports `pr.head_freshness` beside it: `tracked` while a registered poll is re-binding it, `unmaintained` when one is recorded with nothing maintaining it, and null when there is no head at all.
...
The mode is passed explicitly to `bin/fm-brief.sh`, and both values are passed explicitly to `bin/fm-spawn.sh`, `bin/fm-promote.sh`, and the one-call `bin/fm-dispatch.sh` that wraps brief and spawn; each command refuses to guess the values it consumes.
...
A `https://github.com/<owner>/<repo>/pull/<n>` URL requires `gh` and `jq`, is merged only after one live read confirms the pull request is open, not a draft, mergeable, conflict-free, approved at the current head, and green on every unwaived check at that same head, then `gh pr merge` binds that verified head with `--match-head-commit`.
...
An approving review at the current head is what makes a pull request approved, read from the same live view as every other condition; `bin/fm-pr-merge.sh`'s `github_approval_state` owns the rule, and binding the approval to `reviews[].commit.oid` is what stops an approval of a superseded commit from standing in for an approval of what would merge.
It accepts GitHub's own `APPROVED` state and, because that state is unreachable from the single account this fleet posts from, equally accepts the verdict a reviewer writes as the last line of a `COMMENTED` review body ([`docs/verification/pr-review.md`](verification/pr-review.md) records both facts, and `bin/fm-review-verdict-lib.sh` owns the exact line that every program reading or writing it sources).
No flag waives the approval.
GitLab is gated on its own native approvals instead, which it reports without the commit they cover and without any association for the approver, so a GitLab merge discloses both limits, and whether its approver is the account that opened the merge request, rather than implying the GitHub path's guarantees.
