...
Retries of an active hold preserve its hold-set timestamp, while re-holding released work starts a new timestamped lifecycle; a closed task is refused rather than reopened - whether the active backlog still lists it or `done_keep` has retired it to the archive - and `--until` stores the captain's own deferral date through tasks-axi's date gate.
...
Which of the two applies is not a style choice while the work is still running: cleanup owns a live worker's completion transition, so `answer` refuses to close a row whose worker record is still on disk and names `--release` in the refusal.
`reconcile close` is held to the same rule but has no second close mode, so its refusal names the way out that does exist there: stand the worker down first, which leaves the row open and still held for the captain, then reconcile it with the same evidence.
On a scout it also names the captain-call completion gate ahead of cleanup, because a scout is the one kind where cleanup has a prerequisite of its own and a remedy naming only cleanup would be refused in turn.
It names that step by its state and its owner rather than by a command line, the way `bin/fm-teardown.sh`'s own refusal in the same area does: a refusal naming another command is a claim about a second program's reachable states made from a place that cannot see them, and the command-line version of this one was rewritten three times before the shape was recognised as the cause.
...
Every read of a row that may already have been retired sees the archive, not only the gate, and the list is exhaustive rather than illustrative: `answer`, the keyed intake, and `reconcile close` replay a retired row from it, so a replay stays the documented idempotent no-op; `hold` sees an archived row, so the closed-task refusal above holds whatever `done_keep` is set to; the board's reconcile intake calls a retired row closed rather than absent; `open` answers "no longer an open captain call" for one instead of "absent from this backlog", which is what keeps a card the captain can never dismiss off the board; `complete --drop-unrecoverable` resolves the entry it is given through the archive as well, so the drop judges the same row the gate judged; and `reconcile note` distinguishes a retired row, which cannot take a note, from one that is gone.
This list is a rule as well as a description: a read of a possibly-retired row added to `bin/fm-captain-hold.sh` after this sentence belongs in it, and the first commit after the sentence was written broke that rule by adding one and not adding it here.
An archived row is read-only to tasks-axi - `update`, `reopen` and `show` all refuse it - so replay is all any of them offers there, and only for the same decision and the same close.
A captain-held row closed outside this owner therefore records no answer and can never be given one, which strands two readers at once: the gate can never pass, and a pending reconcile request points at a row nothing can act on.
That state is not repaired, because manufacturing a record or skipping what the gate cannot find would clear a call nobody answered.
It is ended instead: every refusal names it plainly and says to raise the call again as its own task, `reconcile close` retires a request whose subject is unrecoverable, and `complete --drop-unrecoverable <task-id>` removes an attested key, refusing unless the row that entry resolves to is archived with no captain answer.
A drop is deliberate, narrow, and recorded - as `decision_dropped=` in the origin's metadata, and as a `dropped` event in the fleet event log, which outlives the cleanup that removes that metadata - while the archived row afterwards retains the closure but not, by itself, the fact that it was retired deliberately.
...
The board's half is pinned in `tests/fm-bearings-board.test.sh`: every published decision card carries exactly one reconcile option, authored options reserve that value across every card type, recommendations name authored options, a decision card whose structured subject appears in the payload's landed rows is dropped while a genuinely open one is kept even when an unrelated landed id contains its key after a newline, and a build serves the board from this home's own server and binds the answer channel before the page that could use it exists.
The board reaches no external tool at all, which that suite proves by building with `lavish-axi` removed from the path rather than stubbed; `tests/fm-bearings-board-render.test.sh` keeps a tripwire in its place so a reintroduced dependency fails by name.
The answer arrives over this home's own socket and is pinned in `tests/fm-board-live.test.sh`; [`verification/process-event-sources.md`](verification/process-event-sources.md) owns the remaining Lavish-adapter evidence, which no longer covers the board.
...
The exact commands and their summarized outputs are recorded in the shipping PR's evidence; run the four suites above plus `tests/fm-send-resolve-key.test.sh`, `tests/fm-bearings-board.test.sh`, `tests/fm-board-live.test.sh`, `tests/fm-procevent.test.sh`, and `bin/fm-lint.sh` to refresh this record.
