...
Every read of a row that may already have been retired follows that rule, not only the gate: `answer`, the keyed intake, and `reconcile close` each replay a retired row out of the archive, so a replay stays the documented idempotent no-op instead of reporting a landed answer as absent from the backlog and leaving a pending reconcile request no command can retire, and `hold` sees an archived row too, so the closed-task refusal above holds whatever `done_keep` is set to.
