...
The board's half is pinned in `tests/fm-bearings-board.test.sh`: every published decision card carries exactly one reconcile option, authored options reserve that value across every card type, recommendations name authored options, a decision card whose structured subject appears in the payload's landed rows is dropped while a genuinely open one is kept even when an unrelated landed id contains its key after a newline, an ordinary build serves the board from this home's own server and binds the answer channel without going near Lavish, a `--lavish` build requires a fresh authoritative listed-open result before binding or arming, nothing a `--lavish` build asks before it establishes the session is a listing - the session-name probe reads `lavish-axi --help` and follows what the installed release advertises in both directions, so it can never consume the read that liveness proof depends on - a reopen retires the pre-reopen source generation and waits for a fresh live listener, and a rebuild of an already-armed board with no live listener starts one.
