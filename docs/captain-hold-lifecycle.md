...
It is ended instead: every refusal names it plainly and says to raise the call again as its own task, `reconcile close` retires a request whose subject is unrecoverable, and `complete --drop-unrecoverable <task-id>` removes an attested key, refusing unless that exact row is archived with no captain answer and recording the removal as `decision_dropped=` beside the keys it leaves.
