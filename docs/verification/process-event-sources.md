...
| board acknowledgement feed | `tests/fm-bearings-board.test.sh` drives an armed Lavish source for this home's own board through the real runner and proves the captured answer records one `acting` acknowledgement per key it named - each id a dispatch order picked on its own row, never the pseudo-key it travelled under - while an otherwise identical Lavish source that is not this board acknowledges nothing. |
