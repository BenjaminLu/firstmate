...
The `diagram-design` skill is a worker-harness dependency bootstrap does not detect and cannot install: a `needs-decision` packet's figures are drawn through that skill rather than hand-written, `bin/fm-packet.sh verify` refuses a packet that owes figures and has none, and a worker whose harness lacks it escalates that as a blocker instead of leaving the figures out; [`bin/fm-packet.sh`'s header](../bin/fm-packet.sh) owns the figure contract.
