...
Each run's test evidence therefore stays on local disk under the no-mistakes home rather than being published to a branch; the review record the captain reads lives on the pull request, where findings and their rulings are posted verbatim.
The orphan `no-mistakes/evidence` branch this setting once published to was removed on 2026-09-19 and is not recreated; the worktree's `.no-mistakes/` stays local and CI rejects tracked entries under that path.
...
`auto` launches `claude --permission-mode auto`, Claude Code's classifier-reviewed permission mode, and is also the posture when the file is absent, so a home configured with nothing runs the posture this fleet actually runs rather than the weaker one.
`bypass` replaces that flag with `--dangerously-skip-permissions`, which skips every permission check; it is a deliberate opt-in a home writes down, never one it inherits by saying nothing.
Every other part of the Claude launch, including its environment prefix, inline settings, model, and effort flags, is the same under either token.
Because a permission posture that is wrong and silent cannot be reviewed, the session-start digest prints one `WORKER LAUNCH POSTURE` line naming the posture in force and whether this file or the shipped default selected it; `bin/fm-claude-launch-lib.sh` resolves it once for both that line and the launch, so the two cannot drift apart.
...
A herdr, zellij, or cmux home is therefore never told `tmux` is missing, and the `treehouse` flag-support upgrade check runs only for the backends that actually use treehouse; that check requires `treehouse get` to advertise both `--lease` and `--no-fetch`, so a treehouse that supports durable leases but not `--no-fetch` reports `MISSING: treehouse`.
...
`FM_OBLIGATION_INTERVAL` (default 900 seconds, `0` to probe on every run) sets how often the forge is actually read, and `FM_OBLIGATION_REPEAT` (default 3600, `0` to disable) is how long an unchanged finding set stays suppressed before it is reported again, because acknowledging a wake is not discharging the obligation.
...
One capture-time seam is narrower still and is scoped to a single artifact: the captain's Bearings board republishes only on consequence-side fleet events, so the capture of that one board's own result also records an acknowledgement per answered key through `bin/fm-bearings-board.sh`, whose header owns that carrier and every rule about it, and no other Lavish session or adapter is touched.
...
FM_CONTRIBUTIONS_BUDGET=20   # 1..25 seconds of forge reads for one contribution poll; not cut to fit FM_CHECK_TIMEOUT, so keep this plus local work inside that bound yourself - a poll the watcher kills loses only the URL it was reading and the rows it never reached: every record it already published survives, and the next poll resumes oldest-first on what it missed; a call that starts with twelve seconds or less of budget left is bounded by that remainder instead of by the twelve-second per-call bound - the last twelve seconds of a poll, where the old five-second bound left only the last five - so a forge that accepts the connection and never answers is never reported as a failure there and those rows only age out as not recently checked; a URL whose observation does not fit the budget at all records that cut-short attempt in its own field rather than being left untouched, and the queue is ordered by the later of the last completed read and the last cut, so it rotates out of the oldest-first head instead of blocking the rows behind it in every later poll; no error is written, so its last good observation stays readable, and because freshness still dates from the last completed read that observation ages out as usual - four calls at the measured 0.9-4.4 seconds each fit this default, but four counts gh invocations and not forge round trips: the comment, review and inline-comment lists each page inside their one invocation at a hundred items per request, and the check rollup pages into a fifth invocation, so the figure assumes a pull request whose comment, review and inline-comment lists each fit one page and whose head commit's rollup fits one page, and one past any of those pages costs more round trips than the figure allows - the header of bin/fm-contributions.sh has the full account
