...
FM_CONTRIBUTIONS_BUDGET=20   # 1..25 seconds of forge reads for one contribution poll; not cut to fit FM_CHECK_TIMEOUT, so keep this plus local work inside that bound yourself - a poll the watcher kills prints nothing, records nothing, and every contribution row ages out to unchecked with no diagnostic
