...
FM_CONTRIBUTIONS_BUDGET=20   # 1..25 seconds of forge reads for one contribution poll; not cut to fit FM_CHECK_TIMEOUT, so keep this plus local work inside that bound yourself - a poll the watcher kills loses only the URL it was reading and the rows it never reached: every record it already published survives, and the next poll resumes oldest-first on what it missed; a budget at or below the twelve-second per-call bound leaves even the first call bounded by the remaining budget, so a forge that accepts the connection and never answers is never reported as a failure and those rows only age out as not recently checked
