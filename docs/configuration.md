...
FM_CONTRIBUTIONS_BUDGET=20   # 1..25 seconds of forge reads for one contribution poll; not cut to fit FM_CHECK_TIMEOUT, so keep this plus local work inside that bound yourself - a poll the watcher kills loses the wake lines it printed and the URL it was reading, while every record it already published survives and the next poll resumes oldest-first on the rows it never reached; a budget at or below the twelve-second per-call bound leaves even the first call bounded by the remaining budget, so a genuinely unreachable forge is never reported as a failure and those rows only age out as not recently checked
