...
`bin/fm-watch.sh` rewrites `state/.last-watcher-beat` once per cycle, immediately before its terminal wait (`event_wait_or_sleep`) as well as at the top of the next cycle, so a healthy watcher's beacon can legitimately age up to `FM_POLL` seconds between writes.
Every guard judges liveness from that mtime alone.
The file's content is the watcher's `<pid> <cycle>`, which carries no liveness meaning: an mtime resolves to whole seconds, so a cycle boundary is otherwise invisible until the second turns over, and the counter is what lets an observer - in practice a test driving the real watcher - see one without waiting a second per cycle.
