...
| `fm-bearings-board.sh`   | Compose the trilingual payload skeleton, then derive, build and arm the stable `/bearings lavish` fleet board, subscribing to this home's fleet events, refresh it in place on a fleet event, and carry the acknowledgement a captain's click leaves on its row |
...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode`), scout, review (`--review <pr-url>`, the reviewer contract), secondmate-charter, and Herdr-lab briefs, with Captain's intent and Firstmate spec subsections on ship/scout/review |
...
| `fm-dispatch.sh`         | One-call intake mechanics: scaffold the brief (`--herdr-lab` forwarded to `fm-brief.sh`) and fill it from `--ask` and `--spec` files, resolve the profile (with a `config/crew-dispatch.json` present, any non-clear resolution stops before filing and asks for explicit `--harness/--model/--effort`), file the backlog item with its repo and `mode=`/`yolo=` note (`kind=review pr=<url>` under `--review`) under a title (`--title`, else the first ask line without its list marker) tasks-axi accepts, and spawn, refusing exactly as each underlying script would - including, before any record, an existing brief that is half-filled or that disagrees with the call's mode or `--herdr-lab` |
...
| `fm-task-progress.sh`    | Project one task's progress - phase plus validation step ladder - from structured state |
