...
| `fm-dispatch.sh`         | One-call intake mechanics: file the backlog item, scaffold and fill the brief from `--ask` and `--spec` files, resolve the profile, and spawn, refusing exactly as each underlying script would |
