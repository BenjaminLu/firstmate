...
| `fm-bearings-board.sh`   | Compose the trilingual payload skeleton, then derive and build the stable `/bearings lavish` fleet board, subscribing to this home's fleet events and served from this home's own server, and carry the acknowledgement a captain's click leaves on its row |
