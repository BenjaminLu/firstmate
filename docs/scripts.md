...
| `fm-dispatch.sh`         | One-call intake mechanics: scaffold the brief (`--herdr-lab` forwarded to `fm-brief.sh`) and fill it from `--ask` and `--spec` files, write the task's design record from the required `--design <file>` or `--no-design <reason>` (scaffolding one an older brief never had), resolve the profile (with a `config/crew-dispatch.json` present, any non-clear resolution stops before filing and asks for explicit `--harness/--model/--effort`), file the backlog item with its repo and `mode=`/`yolo=` note (`kind=review pr=<url>` under `--review`) under a title (`--title`, else the first ask line without its list marker) tasks-axi accepts, and spawn, refusing exactly as each underlying script would - including, before any record, a missing or doubled design choice, an unreadable or empty `--design` file, a blank `--no-design` reason, and an existing brief that is half-filled or that disagrees with the call's mode or `--herdr-lab` |
