...
| `fm-packet.sh`           | Scaffold, verify, and render the worker's decision packet - its context, its figures, and its decision block - beside its task, check one drawing against the figure contract, serve the page from this home's own server, and compose the board's five-question card - the whole packet, drawings included, riding inside it - from it |
