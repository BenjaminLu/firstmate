...
#     install command, written to stdout as well as stderr so a gate that
#     captures either stream alone still shows which tool is missing. When the
#     two streams are the same destination - the usual `2>&1` capture - it is
#     written once, so a merged log does not carry it twice.
#
...
fm_lint_unrunnable() {
  local reason=$1 install=$2 caller message
  caller=${BASH_SOURCE[1]:-fm-lint.sh}
  caller=${caller##*/}
  message=$(printf '%s: LINT NOT RUN (exit %s): %s\n%s: no file was analysed; this is missing lint tooling, not a lint finding.\n%s: install it with: %s' \
    "$caller" "$FM_LINT_UNRUNNABLE" "$reason" "$caller" "$caller" "$install")
  printf '%s\n' "$message"
  [ /dev/fd/1 -ef /dev/fd/2 ] || printf '%s\n' "$message" >&2
