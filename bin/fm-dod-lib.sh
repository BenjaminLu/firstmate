...
# the two answers that end it, and the pass-conditions ask the worker appends.
...
- Ask the reviewer for the pass conditions in the same response you do answer that gate with, in the terms \`ask-user-authority\` step 4 requires of that instruction: append that ask yourself when firstmate's decision does not already carry it, which is the one addition to a decision that is not implementing it.
