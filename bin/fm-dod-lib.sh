...
Apply these four to both, and read \`references/verify-your-fix.md\` in the \`greenlight\` skill at \`~/.claude/skills/greenlight\` for the long form when it is installed where you are running, taking the technique and not its run structure, which assumes the agent opens an umbrella pull request and a sub-pull-request per finding and does not apply here.
