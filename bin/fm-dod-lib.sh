...
The captain reads EN / 繁體 / 简体, and the board opens your whole packet inside his decision card, so every line of "What only this session knows" is written in all three - \`- en:\`, \`- hant:\` and \`- hans:\` lines, one item - and so are a figure's heading and caption, through \`heading.hant:\`, \`heading.hans:\`, \`caption.hant:\` and \`caption.hans:\`. You write them because nobody else knows what you meant; verify refuses the half of a line you leave out.
