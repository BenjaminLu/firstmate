...
# 1..25). Each gh call is bounded by five seconds and by the remaining budget:
# no call runs past five seconds, and none is bounded below five while budget
# remains. A read killed at the poll's own deadline is unmeasured; one killed
# at the five-second bound with budget to spare is a genuine forge failure.
# Oldest observations go first, so a large corpus progresses across polls, and
# within one timestamp an attempt that recorded nothing goes before a completed
# observation, so the same tail cannot be the cut-short one every poll.
