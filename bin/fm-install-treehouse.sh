...
# The floor is v2.2.0, the release that added `treehouse get --no-fetch`:
# bin/fm-spawn.sh sends that flag unconditionally and bin/fm-bootstrap.sh
# reports an older treehouse as MISSING, so an older pin makes every spawning
# E2E script time out waiting for a slot it never gets.
set -eu
